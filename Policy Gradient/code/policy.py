...
class GaussianPolicy(BasePolicy, nn.Module):
    def __init__(self, network, action_dim):
        ...
        self.log_std=nn.Parameter(torch.zeros(size=[action_dim]))
    ...
    def action_distribution(self, observations):
        ...
        # dist=torch.distributions.normal.Normal(loc=self.network(observations), scale=self.std())
        # dist=torch.distributions.Independent(dist, 1)
        dist=torch.distributions.MultivariateNormal(loc=self.network(observations), scale_tril=torch.diag(self.std()))
        return dist
