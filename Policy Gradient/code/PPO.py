...
class PolicyGradient(object):
    ...
    def __init__(self, env, config, seed, logger=None):
        ...
        if config.use_actor_critic:
            ...
            self.value_network = Critic(self.env, self.config)
    ...
    def eval(self, batch_observations, batch_actions):
        value=self.value_network.forward(batch_observations)
        # mean=self.Actor(batch_observations)
        dist=self.policy.action_distribution(batch_observations)
    ...
    def sample_path(self, env, num_episodes=None):
        ...
        while num_episodes or t < self.config.batch_size:
            state = env.reset()
            states, actions, rewards, values, log_probas = [], [], [], [], []
            episode_reward = 0

            for step in range(self.config.max_ep_len):
                states.append(state)
                action, log_proba = self.policy.act(states[-1][None])[0]
    ...
    def calculate_advantage(self, returns, observations):
        ...
        if self.config.use_baseline:
            ...
            adtantages= returns - self.value_network.forward(observations)
        else:
            advantages = returns
    ...
    def train(self):
        ...
        for t in range(self.config.num_batches):
            ...
            if self.value_network:
                self.value_network.update_value()
